...
<!-- Nav tabs -->
<a href="/"><img alt="PM Core" height="45" src="https://www.processmaker.io/img/logo-black.svg" style="position: absolute;">
    </a>
<ul id="myTabs" class="nav nav-tabs" role="tablist">

    <li role="presentation" class="mvp"><a href="#mvp" aria-controls="nda" role="tab" data-toggle="tab">Onboarding</a></li>
    <li role="presentation"><a href="#nda" aria-controls="nda" role="tab" data-toggle="tab">NDA</a></li>
    <li role="presentation" class="slack"><a href="#slack" aria-controls="slack" role="tab" data-toggle="tab">Slack</a></li>
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
          <li<?php echo (preg_match('/readme.html/',$_SERVER['REQUEST_URI'] || empty($_SERVER['QUERY_STRING']))?' class="active"':'');?>><a href="/readme.html/">Readme</a></li>
    ...
    <div role="tabpanel" class="tab-pane" id="nda">
        <nav class="navbar navbar-default">
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav">
                        <li<?php echo (preg_match('/readme.html/',$_SERVER['REQUEST_URI'] || $_SERVER['QUERY_STRING'] == 'example=nda&user=Test')?' class="active"':'');?>><a href="/readme.html?example=nda">Readme</a></li>
    ...
    <div role="tabpanel" class="tab-pane" id="slack">
        <nav class="navbar navbar-default">
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav">
                        <li<?php echo (preg_match('/readme.html/',$_SERVER['REQUEST_URI'] || $_SERVER['QUERY_STRING'] == 'example=slack&user=Test')?' class="active"':'');?>><a href="/readme.html?example=slack">Readme</a></li>
                        <li<?php echo (preg_match('/import_form.html/',$_SERVER['REQUEST_URI'])?' class="active"':'');?>><a href="/import_form.html?example=slack">Import</a></li>
                        <li<?php echo (preg_match('/processes_form.html/',$_SERVER['REQUEST_URI'])?' class="active"':'');?>><a href="/processes_form.html?example=slack">Process List</a></li>
                    </ul>
...
<script type="text/javascript">
    $('ul#myTabs li a').on('click', function (e) {
        ...
        if (link == '#nda') window.location.replace('/?example=nda&user=Test');
        else if (link == '#slack') window.location.replace('/?example=slack&user=Test');
        else if (link == '#mvp') window.location.replace('/');
    });

    <?php if (isset($_GET['example']) && $_GET['example'] == 'nda') {
        echo '$(\'#myTabs a[href="#nda"]\').tab(\'show\');';

    } elseif (isset($_GET['example']) && $_GET['example'] == 'slack') {
            echo '$(\'#myTabs a[href="#slack"]\').tab(\'show\');';
        } else
